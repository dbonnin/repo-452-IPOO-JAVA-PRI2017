...
create table operaciones(
	nro_operacion serial,
	fecha_hora datetime,
	imagen blob
	id_cajero bigint references cajeros(id_cajero),
	id_acceso references accesos(id_acceso)
	estado varchar(32),
	mensaje text
);
...
create table transacciones(
	nro_operacion primary key references operaciones(nro_operacion),
	importe numeric(18, 2),
	id_moneda int references monedas(id_moneda)
);

create table transferencias(
	nro_operacion primary key references transacciones(nro_operacion),
	nro_cuenta_origen references cuentas(nro_cuenta),
	nro_cuenta_destino references cuentas(nro_cuenta)
);